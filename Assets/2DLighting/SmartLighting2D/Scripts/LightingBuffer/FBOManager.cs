...
public class FBOManager : MonoBehaviour
{
    ...
    public void Awake() {
		...
		if (Lighting2D.lightingSourceSettings.fixedLightBufferSize && Lighting2D.lightingSourceSettings.lightingBufferPreload) {
			for(int i = 1; i <= Lighting2D.lightingSourceSettings.lightingBufferPreloadCount; i++) {
				AddBuffer(LightingRender.GetTextureSize(Lighting2D.lightingSourceSettings.fixedLightTextureSize), null);
			}
		}
    }
    ...
    static public FBOManager Get() {
		...
		setFBOManager.name = "Light Buffers";
	...
	static public LightingBuffer2D AddBuffer(int textureSize, LightingSource2D light) {
		...
		FBOManager fboManager = FBOManager.Get();
